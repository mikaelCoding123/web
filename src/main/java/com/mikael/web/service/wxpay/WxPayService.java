...
public class WxPayService extends PayFilterService implements PayWayService {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(calculateRedEnvelopes(100, 5));
...
//        }
    ...
    public static List<Double> calculateRedEnvelopes(double totalAmount, int totalPeople) {
        List<Double> amounts = new ArrayList<>();
        double remainingAmount = totalAmount;

        for (int i = 1; i <= totalPeople; i++) {
            double amount = totalAmount * (2 * i - 1) / (2.0 * totalPeople);
            amounts.add(amount);
            remainingAmount -= amount;
        }

        // 最后一个用户获得剩余的全部金额（理论上应为0，因为前面的计算已经确保总和为totalAmount）
        if (remainingAmount > 0) {
            amounts.set(totalPeople - 1, amounts.get(totalPeople - 1) + remainingAmount);
        }
